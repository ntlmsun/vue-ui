 ...
 * @param {*} wbBytes
 */
export const getCode = (wbBytes) => {
  let wbCode = [''];
  for (let i = 0; i < wbBytes.length; i++) {
    const bytes = wbBytes[i];
    if (bytes.length === 1) {
      for (let k = 0; k < wbCode.length; k++) {
        wbCode[k] += bytes;
      }
    } else {
      const wbCodeList = wbCode.slice(0);
      wbCode = [];
      for (let k = 0; k < bytes.length; k++) {
        const targetCode = wbCodeList.slice(0);
        for (let j = 0; j < targetCode.length; j++) {
          targetCode[j] += bytes.charAt(k);
        }
        wbCode = wbCode.concat(targetCode);
      }
    }
  }
  return wbCode.toString().replace(/\s+/g, '');
};
 ...
 * @param {*} bytes
 */
export const getWbCode = (bytes) => {
  if (!/[\u4e00-\u9fa5]/gm.test(bytes)) {
    return bytes;
  }
  for (let i = 0; i < wbLib.length; i++) {
    const bytesList = wbLib[i];
    for (let k = 0; k < bytesList.length; k++) {
      if (bytes === bytesList[k]) {
        return bytesList[0];
      }
    }
  }
