...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`msun.datepicker.weeks.${ week }`)),
    dayNames: weeks.map(week => t(`msun.datepicker.weeks.${ week }`)),
    monthNamesShort: months.map(month => t(`msun.datepicker.months.${ month }`)),
    monthNames: months.map((month, index) => t(`msun.datepicker.month${ index + 1 }`)),
    amPm: ['am', 'pm']
  };
};
