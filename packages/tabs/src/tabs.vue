...
export default {
  ...
  render(h) {
    ...
    const header = (
      <div class={['ms-tabs__header', `is-${tabPosition}`]}>
        {newButton}
        <tab-nav {...navData}></tab-nav>
      </div>
    );
    const panels = <div class="ms-tabs__content">{this.$slots.default}</div>;

    return (
      <div
        class={{
          'ms-tabs': true,
          'ms-tabs--card': type === 'card',
          [`ms-tabs--${tabPosition}`]: true,
          'ms-tabs--border-card': type === 'border-card'
        }}
      >
