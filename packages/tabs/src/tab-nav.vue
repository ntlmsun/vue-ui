...
export default {
  ...
  render(h) {
    ...
    const scrollBtn = scrollable
      ? [
          <span class={['ms-tabs__nav-prev', scrollable.prev ? '' : 'is-disabled']} on-click={scrollPrev}>
            <i class="ms-icon-arrow-left"></i>
          </span>,
          <span class={['ms-tabs__nav-next', scrollable.next ? '' : 'is-disabled']} on-click={scrollNext}>
    ...
    return (
      <div class={['ms-tabs__nav-wrap', scrollable ? 'is-scrollable' : '', `is-${this.rootTabs.tabPosition}`]}>
        {scrollBtn}
        <div class={['ms-tabs__nav-scroll']} ref="navScroll">
          <div class={['ms-tabs__nav', `is-${this.rootTabs.tabPosition}`, stretch && ['top', 'bottom'].indexOf(this.rootTabs.tabPosition) !== -1 ? 'is-stretch' : '']} ref="nav" style={navStyle} role="tablist" on-keydown={changeTab}>
