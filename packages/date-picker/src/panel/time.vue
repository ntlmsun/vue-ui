<template>
  <transition name="ms-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div v-show="visible" class="ms-time-panel ms-popper" :class="popperClass">
      <div class="ms-time-panel__content" :class="{ 'has-seconds': showSeconds }">
      ...
      <div class="ms-time-panel__footer">
        ...
        <button type="button" class="ms-time-panel__btn" :class="{confirm: !disabled}" @click="handleConfirm()">{{ t('msun.datepicker.confirm') }}</button>
      </div>
    </div>
  </transition>
</template>
...
import { limitTimeRange, isDate, clearMilliseconds, timeWithinRange } from 'msun-lib-ui/shared/utils/date-util';
