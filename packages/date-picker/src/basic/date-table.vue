<template>
  <table cellspacing="0" cellpadding="0" class="ms-date-table" @click="handleClick" @mousemove="handleMouseMove" :class="{ 'is-week-mode': selectionMode === 'week' }">
    <tbody>
      <tr>
        <th v-if="showWeekNumber">{{ t('msun.datepicker.week') }}</th>
        <th v-for="(week, key) in WEEKS" :key="key">{{ t('msun.datepicker.weeks.' + week) }}</th>
      </tr>
      <tr class="ms-date-table__row" v-for="(row, key) in rows" :class="{ current: isWeekActive(row[1]) }" :key="key">
...
<script>
import { getFirstDayOfMonth, getDayCountOfMonth, getWeekNumber, getStartDateOfMonth, prevDate, nextDate, isDate, clearTime as _clearTime } from 'msun-lib-ui/shared/utils/date-util';
import Locale from 'msun-lib-ui/shared/mixins/locale';
import { arrayFindIndex, arrayFind, coerceTruthyValueToArray } from 'msun-lib-ui/shared/utils/util';
