...
export default {
  ...
  watch: {
    visible(val) {
      this.broadcast('MsDropdownMenu', 'visible', val);
      this.$emit('visible-change', val);
    },
    focusing(val) {
      const selfDefine = this.$el.querySelector('.ms-dropdown-selfdefine');
      if (selfDefine) {
        // 自定义
        if (val) {
  ...
  methods: {
    ...
    initAria() {
      ...
      if (!this.splitButton) {
        ...
        this.triggerElm.setAttribute('class', (this.triggerElm.getAttribute('class') || '') + ' ms-dropdown-selfdefine'); // 控制
      }
    },
    initEvent() {
  ...
  render(h) {
    ...
    let triggerElm = !splitButton ? (
      ...
      <ms-button-group>
        <ms-button type={type} size={dropdownSize} nativeOn-click={handleMainButtonClick}>
          {this.$slots.default}
        </ms-button>
        <ms-button ref="trigger" type={type} size={dropdownSize} class="ms-dropdown__caret-button">
          <i class="ms-dropdown__icon ms-icon-arrow-down"></i>
        </ms-button>
      </ms-button-group>
    );

    return (
      <div class="ms-dropdown" v-clickoutside={hide}>
        {triggerElm}
        {this.$slots.dropdown}
      </div>
    );
