import { arrayFindIndex } from 'msun-lib-ui/shared/utils/util';
import { getCell, getColumnByCell, getRowIdentity } from './util';
import { getStyle, hasClass, removeClass, addClass } from 'msun-lib-ui/shared/utils/dom';
import MsCheckbox from 'msun-lib-ui/packages/checkbox';
import MsTooltip from 'msun-lib-ui/packages/tooltip';
...
export default {
  ...
  render(h) {
    ...
    return (
      <table class="ms-table__body" cellspacing="0" cellpadding="0" border="0">
        <colgroup>
        ...
        <tbody>
          ...
          <ms-tooltip effect={this.table.tooltipEffect} placement="top" ref="tooltip" content={this.tooltipContent} />
        </tbody>
      </table>
    );
  },
  ...
  watch: {
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.ms-table__row');
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      const classes = ['ms-table__row'];
      if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
        classes.push('current-row');
      }

      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('ms-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'ms-tooltip') && cellChild.childNodes.length)) {
        return;
      }
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push('ms-table__row--level-' + treeRowData.level);
      ...
      return (
        <tr style={[displayStyle, this.getRowStyle(row, $index)]} class={rowClasses} key={this.getKeyOfRow(row, $index)} on-dblclick={$event => this.handleDoubleClick($event, row)} on-click={$event => this.handleClick($event, row)} on-contextmenu={$event => this.handleContextMenu($event, row)} on-mouseenter={_ => this.handleMouseEnter($index)} on-mouseleave={this.handleMouseLeave}>
          {columns.map((column, cellIndex) => {
            ...
            return (
              <td style={this.getCellStyle($index, cellIndex, row, column)} class={this.getCellClass($index, cellIndex, row, column)} rowspan={rowspan} colspan={colspan} on-mouseenter={$event => this.handleCellMouseEnter($event, row)} on-mouseleave={this.handleCellMouseLeave}>
    ...
    wrappedRowRender(row, $index) {
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        if (!renderExpanded) {
          console.error('[Msun Error]renderExpanded is required.');
          return tr;
        }
        // 使用二维数组，避免修改 $index
        return [
          [
            ...
            <tr key={'expanded-row__' + tr.key}>
              <td colspan={this.columnsCount} class="ms-table__expanded-cell">
