<template>
  <div :class="['ms-cascader-panel', border && 'is-bordered']" @keydown="handleKeyDown">
...
import { noop, coerceTruthyValueToArray, isEqual, isEmpty, valueEquals } from 'msun-lib-ui/shared/utils/util';
...
const getSibling = (el, distance) => {
  const { parentNode } = el;
  if (parentNode) {
    const siblings = parentNode.querySelectorAll('.ms-cascader-node[tabindex="-1"]');
  ...
  methods: {
    ...
    handleKeyDown(e) {
      ...
      switch (keyCode) {
        ...
        case KeyCode.left:
          ...
          if (preMenu) {
            const expandedNode = preMenu.$el.querySelector('.ms-cascader-node[aria-expanded="true"]');
        ...
        case KeyCode.right:
          ...
          if (nextMenu) {
            const firstNode = nextMenu.$el.querySelector('.ms-cascader-node[tabindex="-1"]');
            focusNode(firstNode);
          }
          break;
        case KeyCode.enter:
    ...
    scrollIntoView() {
      ...
      menus.forEach(menu => {
        ...
        if (menuElement) {
          const container = menuElement.querySelector('.ms-scrollbar__wrap');
          const activeNode = menuElement.querySelector('.ms-cascader-node.is-active') || menuElement.querySelector('.ms-cascader-node.in-active-path');
