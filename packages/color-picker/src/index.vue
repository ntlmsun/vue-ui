  ...
  <div :class="[
    ...
    <div class="ms-color-picker__trigger" @click="handleTrigger">
      <span class="ms-color-picker__color" :class="{ 'is-alpha': showAlpha }">
        <span class="ms-color-picker__color-inner" :style="{
    ...
    <picker-dropdown ref="dropdown" :class="['ms-color-picker__panel', popperClass || '']" v-model="showPicker" @pick="confirmValue" @clear="clearValue" :color="color" :show-alpha="showAlpha" :predefine="predefine">
    </picker-dropdown>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    clearValue() {
      ...
      if (this.value !== null) {
        this.dispatch('MsFormItem', 'el.form.change', null);
      }
      this.showPanelColor = false;
      this.showPicker = false;
      this.resetColor();
