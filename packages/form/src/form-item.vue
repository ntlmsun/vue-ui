<template>
  <div class="ms-form-item" :class="[{
    ...
    <div class="ms-form-item__content" :style="contentStyle">
      <slot></slot>
...
import objectAssign from 'msun-lib-ui/shared/utils/merge';
import { noop, getPropByPath } from 'msun-lib-ui/shared/utils/util';
...
export default {
  ...
  computed: {
    ...
    form() {
      ...
      while (parentName !== 'MsForm') {
        if (parentName === 'MsFormItem') {
          this.isNested = true;
        }
    ...
    _formSize() {
      return this.msForm.size;
    },
    msFormItemSize() {
      return this.size || this._formSize;
    },
    sizeClass() {
      return this.msFormItemSize || (this.$MSUN || {}).size;
    }
  },
  data() {
    return {
  ...
  methods: {
    validate(trigger, callback = noop) {
      ...
      validator.validate(model, { firstFields: true }, (errors, invalidFields) => {
        ...
        this.msForm && this.msForm.$emit('validate', this.prop, !errors, this.validateMessage || null);
      });
    },
    clearValidate() {
      this.validateState = '';
    ...
    resetField() {
      ...
      this.broadcast('MsTimeSelect', 'fieldReset', this.initialValue);
    },
    getRules() {
  ...
  mounted() {
    if (this.prop) {
      this.dispatch('MsForm', 'el.form.addField', [this]);
  ...
  beforeDestroy() {
    this.dispatch('MsForm', 'el.form.removeField', [this]);
  }
};
</script>
